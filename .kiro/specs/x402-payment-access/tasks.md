...
- [ ] 3.4 支払い coordinator と terminal semantics を実装する
  ...
  - canonical gate へ payment success/error/cancel を返す callback seam を提供するが、WebMCP lifecycle は呼ばない。
  - 完了時、mock の成功・拒否・cancel・timeout の各 flow が一つの terminal state に到達する。
  - _Requirements: 2.1, 2.2, 2.4, 3.1, 3.2, 3.4, 3.5, 4.1, 4.2, 4.3, 4.4, 4.5_
...
- [ ] 5.1 Server の 402、決済、CORS、idempotency integration test を完成させる
  ...
  - 完了時、server test command が実ネットワークなしで全 critical paid-path case を成功させる。
  - _Requirements: 1.1, 1.2, 1.3, 3.1, 3.2, 3.3, 3.4, 3.5, 4.3, 4.5, 5.1, 5.2, 5.3, 5.4, 5.5, 6.5_
  - _Boundary: ServerPaymentTestHarness_
...
- [ ] 5.2 (P) Browser の consent、network、retry、recovery integration test を完成させる
  ...
  - _Requirements: 1.4, 1.5, 2.1, 2.2, 2.3, 2.4, 2.5, 3.1, 3.2, 3.3, 3.4, 3.5, 4.1, 4.2, 4.3, 4.4, 4.5, 5.5, 6.5_
  - _Boundary: BrowserPaymentTestHarness_
  - _Depends: 4.1_
...
- [ ] 5.3 Base Sepolia-only invariant と package validation を最終確認する
  ...
  - app 間 runtime import がなく、payment code が sponsor または WebMCP lifecycle を所有していないことを確認する。
  - 完了時、全 validation が成功し、後続 gate integration が payment callback と canonical evidence だけを利用できる。
  - _Requirements: 1.2, 1.3, 2.5, 5.5, 6.2, 6.3, 6.4, 6.5_
  - _Boundary: PaymentPolicy, PaymentProtection, PaymentCoordinator, LocalFacilitatorCompatibility_
  - _Depends: 5.1, 5.2_
